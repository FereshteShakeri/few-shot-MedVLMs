...
class KGCOOP(FSCLIPmethod):
    ...
    def forward(self,
        ...
        coop_model = CustomCLIP(self.cfg, dtype, classnames, model.cuda()).cuda()
        ...
        for train_idx in range(self.cfg['train_epoch']):
            ...
            for i, (images, target) in enumerate(tqdm(train_loader)):
                ...
                if self.cfg["PREC"] == "amp":
                    with autocast():
                        image_features = model.encode_image(images)
                        image_features /= image_features.norm(dim=-1, keepdim=True)
                        output, score = coop_model(image_features)
                    ...
                    image_features /= image_features.norm(dim=-1, keepdim=True)
                    output, score = coop_model(image_features)
                    loss = F.cross_entropy(output, target)+8.0*score
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
                    scheduler.step()
            ...
            output, score = coop_model(val_features)
        ...
        output, score = coop_model(test_features)
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, dtype, classnames, clip_model):
        ...
        temp =  "a histopathology slide showing {}"
