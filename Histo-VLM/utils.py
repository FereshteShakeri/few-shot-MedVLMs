...
def average_task_results(list_results):
    ...
    for iMetric in metrics_name:
        values = np.concatenate([np.expand_dims(np.array(task[iMetric]), -1) for task in list_results], -1)
...
def save_results(metrics, out_path, id_experiment=None, id_metrics=None):
    ...
    out_path = os.path.join(out_path, id_experiment)
    # Create main experiment folder
    if not os.path.isdir(out_path):
        os.mkdir(out_path)

    # Store metrics in experiment dataset
    with open(out_path + '/metrics_' + id_metrics + '.json', 'w') as fp:
